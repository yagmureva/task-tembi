...
// Use Nuxt's useFetch composable to fetch quotes from ZenQuotes API
const {
  data,
  error,
  isFetching, // Use isFetching instead of pending
  refresh: refetch,
} = await useFetch("https://zenquotes.io/api/random");

// Handle errors
if (error.value) {
  console.error("Error fetching quote:", error.value);
}
</script>
